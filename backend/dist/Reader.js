...
exports.default =
	exports.TAG_ISO_14443_4 =
	exports.TAG_ISO_14443_3 =
	exports.KEY_TYPE_B =
	exports.KEY_TYPE_A =
	exports.CONNECT_MODE_DIRECT =
	exports.CONNECT_MODE_CARD =
		void 0;
...
const CONNECT_MODE_DIRECT = (exports.CONNECT_MODE_DIRECT =
	"CONNECT_MODE_DIRECT");
...
class Reader extends _events.default {
	...
	connect(mode = CONNECT_MODE_CARD) {
		...
		return new Promise((resolve, reject) => {
			...
			this.reader.connect(
				...
				(err, protocol) => {
					if (err) {
						const error = new _errors.ConnectError(
							_errors.FAILURE,
	...
	disconnect() {
		if (!this.connection) {
			throw new _errors.DisconnectError(
				"not_connected",
		...
		return new Promise((resolve, reject) => {
			...
			this.reader.disconnect(this.reader.SCARD_LEAVE_CARD, (err) => {
				if (err) {
					const error = new _errors.DisconnectError(
						_errors.FAILURE,
	...
	transmit(data, responseMaxLength) {
		if (!this.card || !this.connection) {
			throw new _errors.TransmitError(
				_errors.CARD_NOT_CONNECTED,
		...
		return new Promise((resolve, reject) => {
			...
			this.reader.transmit(
				...
				(err, response) => {
					if (err) {
						const error = new _errors.TransmitError(
							_errors.FAILURE,
	...
	control(data, responseMaxLength) {
		if (!this.connection) {
			throw new _errors.ControlError(
				"not_connected",
		...
		return new Promise((resolve, reject) => {
			...
			this.reader.control(
				...
				(err, response) => {
					if (err) {
						const error = new _errors.ControlError(
							_errors.FAILURE,
	...
	async loadAuthenticationKey(keyNumber, key) {
		...
		if (!Buffer.isBuffer(key) && !Array.isArray(key)) {
			if (typeof key !== "string") {
				throw new _errors.LoadAuthenticationKeyError(
					"invalid_key",
	...
	async authenticate(blockNumber, keyType, key, obsolete = false) {
		...
		if (statusCode !== 0x9000) {
			...
			throw new _errors.AuthenticationError(
				_errors.OPERATION_FAILED,
				`Authentication operation failed: Status code: 0x${statusCode.toString(16)}`,
			);
		}
		return true;
	}
	async read(
		...
		if (statusCode !== 0x9000) {
			throw new _errors.ReadError(
				_errors.OPERATION_FAILED,
				`Read operation failed: Status code: 0x${statusCode.toString(16)}`,
			);
		}
	...
	async write(blockNumber, data, blockSize = 4) {
		...
		if (statusCode !== 0x9000) {
			throw new _errors.WriteError(
				_errors.OPERATION_FAILED,
				`Write operation failed: Status code: 0x${statusCode.toString(16)}`,
			);
		}
		return true;
	}
	...
	async handle_Iso_14443_3_Tag() {
		...
		try {
			...
			if (response.length < 2) {
				const error = new _errors.GetUIDError(
					"invalid_response",
	...
	async handle_Iso_14443_4_Tag() {
		...
		try {
			...
			if (response.length === 2 && response.readUInt16BE(0) === 0x6a82) {
				const err = new Error(
					`Not found response. Tag not compatible with AID ${aid.toString("hex").toUpperCase()}.`,
				);
				this.emit("error", err);
				return;
			}
